...
class PantoneRGBConverter:
    ...
    def get_text_color_for_background(self, rgb: tuple[int, int, int]) -> str:
        ...
        return (
            "\033[97m" if luminance < 0.5 else "\033[30m"
        )  # White text for dark bg, black for light

    def print_color_swatch_terminal(
        self, color_name: str, rgb: tuple[int, int, int], hex_val: str
    ) -> None:
        ...
        if sys.stdout.isatty():  # Check if output is to terminal
            ...
            print(
                f"{swatch} {color_name:<20} RGB({rgb[0]:3}, {rgb[1]:3}, {rgb[2]:3})  {hex_val}"
            )
        else:
            # Fallback for non-terminal output
            print(
                f"■ {color_name:<20} RGB({rgb[0]:3}, {rgb[1]:3}, {rgb[2]:3})  {hex_val}"
            )

    def print_color_swatch_colorama(
        self, color_name: str, rgb: tuple[int, int, int], hex_val: str
    ) -> None:
        ...
        for color_rgb, back_color in color_map.items():
            distance = sum((a - b) ** 2 for a, b in zip(rgb, color_rgb, strict=False))
    ...
    def create_color_palette_image(
        self, output_filename: str = "university_palette.png"
    ) -> None:
        ...
        img_height = (
            rows * (swatch_size + text_height + margin) + margin + 100
        ...
        # Draw title
        draw.text(
            (margin, margin), "University Color Palette", fill="black", font=title_font
        )
        ...
        for category, colors in palette.items():
            # Category title
            draw.text(
                (x, y), category.replace("_", " ").title(), fill="black", font=font
            )
            y += 20
            ...
            for color_name, values in colors.items():
                ...
                # Draw color info
                draw.text(
                    (x, y + swatch_size + 5), color_name[:15], fill="black", font=font
                )
                draw.text(
                    (x, y + swatch_size + 20), f"RGB{rgb}", fill="black", font=font
                )
    ...
    def create_matplotlib_palette(self) -> None:
        ...
        for idx, (category, colors) in enumerate(palette.items()):
            ...
            for i, (color_name, values) in enumerate(colors.items()):
                ...
                # Add text labels
                text_color = (
                    "white" if sum(rgb) < 384 else "black"
