        ...
        if self.user_type == "student" and self.email:
            if not self.email.endswith("@mun.ca"):
                raise ValidationError(
                    {"email": "Student email must be from @mun.ca domain"}
                )
...
class StudentProfile(models.Model):
    ...
    academic_year = models.CharField(
        max_length=10, choices=ACADEMIC_YEAR_CHOICES, blank=True
    )
    university = models.CharField(
        max_length=200, default="Memorial University of Newfoundland"
    )
    ...
    )  # Store LinkedIn, GitHub, portfolio URLs
    ...
    )  # Store external links with metadata

    # Availability
    availability = models.JSONField(
        default=list, blank=True
    )  # Store selected availability options
    currently_available = models.CharField(
        max_length=10, choices=CURRENT_AVAILABILITY_CHOICES, blank=True
    )
    ...
    remote_preference = models.CharField(
        max_length=10, choices=REMOTE_PREFERENCE_CHOICES, blank=True
    )
    location_flexibility = models.CharField(
        max_length=15, choices=LOCATION_FLEXIBILITY_CHOICES, blank=True
    )
...
class Reference(models.Model):
    ...
    phone = models.CharField(max_length=20, blank=True)
    skill_endorsements = models.TextField(
...
class Project(models.Model):
    ...
    employer = models.ForeignKey(
        EmployerProfile, on_delete=models.CASCADE, related_name="projects"
    )
    ...
    min_academic_year = models.CharField(
        max_length=10, choices=StudentProfile.ACADEMIC_YEAR_CHOICES, blank=True
    )
    preferred_programs = models.JSONField(
        default=list
    )  # Store list of preferred academic programs
