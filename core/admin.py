...
class UserAdmin(BaseUserAdmin):
    ...
    if fieldsets is not None:
        result = list(fieldsets) + list(additional_info)
    else:
        result = list(additional_info)
...
class EmployerProfileAdmin(admin.ModelAdmin):
    ...
    def approve_employers(
        self, request: HttpRequest, queryset: QuerySet[EmployerProfile]
    ) -> None:
        queryset.update(approval_status="approved")

    approve_employers.short_description = "Approve selected employers"  # type: ignore[attr-defined]

    def reject_employers(
        self, request: HttpRequest, queryset: QuerySet[EmployerProfile]
    ) -> None:
        queryset.update(approval_status="rejected")

    reject_employers.short_description = "Reject selected employers"  # type: ignore[attr-defined]
