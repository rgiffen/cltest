...
from .models import StudentProfile, Project, Skill
import re
...
class ProjectMatcher:
    ...
    def __init__(self):
        self.skill_synonyms = {
            'javascript': ['js', 'javascript', 'node.js', 'nodejs', 'react', 'vue', 'angular'],
            'python': ['python', 'django', 'flask', 'fastapi', 'pandas', 'numpy'],
            'web_development': ['html', 'css', 'frontend', 'backend', 'fullstack', 'web dev'],
            'database': ['sql', 'mysql', 'postgresql', 'mongodb', 'database', 'db'],
            'mobile': ['ios', 'android', 'swift', 'kotlin', 'react native', 'flutter'],
            'data_science': ['data analysis', 'machine learning', 'ml', 'ai', 'analytics'],
        }
    
    def find_matches(self, project: Project, limit: int = 20) -> List[Tuple[StudentProfile, Dict]]:
        """
        ...
        ).select_related('user').prefetch_related('skills', 'education', 'employment')
    ...
    def _calculate_match(self, student: StudentProfile, project: Project) -> Dict:
    ...
    def _match_skills(self, student: StudentProfile, project: Project, match_data: Dict) -> float:
        ...
        for req_skill in required_skills:
            if self._skill_matches(req_skill, student_skills):
                required_matches += 1
                match_data['evidence'].append(f"✓ Has required skill: {req_skill.title()}")
        ...
        for pref_skill in preferred_skills:
            if self._skill_matches(pref_skill, student_skills):
                ...
                match_data['match_reasons'].append(f"Preferred skill match: {pref_skill}")
        
        # Calculate skills score
        if required_total > 0:
            required_score = (required_matches / required_total) * 80  # 80% for required
            preferred_bonus = (preferred_matches / max(preferred_total, 1)) * 20  # 20% bonus
    ...
    def _match_availability(self, student: StudentProfile, project: Project, match_data: Dict) -> float:
        """Match student availability with project work type and duration"""
        score = 50  # Base score
        
        # Check work type compatibility
        if project.work_type == 'remote' and student.remote_preference in ['remote', 'hybrid', 'flexible']:
            score += 25
            match_data['evidence'].append("✓ Remote work compatible")
        elif project.work_type == 'onsite' and student.remote_preference in ['onsite', 'flexible']:
            score += 25
    ...
    def _match_academic_level(self, student: StudentProfile, project: Project, match_data: Dict) -> float:
        ...
        if student_level >= required_level:
            score = 100
            match_data['evidence'].append(f"✓ Academic level: {student.get_academic_year_display()}")
            if student_level > required_level:
                match_data['match_reasons'].append("Exceeds minimum academic requirements")
        else:
            score = max(0, 50 - (required_level - student_level) * 15)
            match_data['evidence'].append(f"⚠ Academic level below minimum requirement")
        
        return score
    
    def _match_experience(self, student: StudentProfile, project: Project, match_data: Dict) -> float:
        ...
        if total_employment > 0:
            ...
            match_data['evidence'].append(f"✓ {total_employment} work experience entries")
        
        if relevant_projects > 0:
            score += 25
            match_data['evidence'].append(f"✓ {relevant_projects} relevant project experience")
            ...
            for education in student.education.all():
                for preferred_program in project.preferred_programs:
                    if preferred_program.lower() in education.field_of_study.lower():
                        score += 15
                        match_data['evidence'].append(f"✓ Relevant education: {education.field_of_study}")
                        break
        
        return min(100, score)


def get_project_matches(project_id: int) -> List[Tuple[StudentProfile, Dict]]:
...
def get_student_projects(student_profile: StudentProfile) -> List[Tuple[Project, Dict]]:
